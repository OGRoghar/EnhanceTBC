...
function API.EnterEditMode(scope)
  ...
  if not ready then return false, "addon not ready" end
  if not (ETBC.Mover and ETBC.Mover.SetMoveMode) then return false, "edit mode unavailable" end
  ETBC.Mover:SetMoveMode(true)
  if ETBC.PublicAPIInternal and ETBC.PublicAPIInternal.OnEditModeChanged then
    ETBC.PublicAPIInternal.OnEditModeChanged(true, tostring(scope or "all"))
  end
  return true
end
